...
| Nx | `22.7.5` | monorepo runner: `@nx/next`, `@nx/jest`, `@nx/eslint`, `@nx/webpack` |
| Package manager | pnpm workspaces | phantom-dep discipline required (see §7) |
...
```bash
# Core
pnpm add next@~16.1.6 react@^19 react-dom@^19 \
...
# Testing
pnpm add -D jest@^29 jest-environment-jsdom ts-jest \
...
3. **No phantom dependencies.** pnpm does not hoist transitives — every imported package must be a *direct* dependency, or the webpack production build breaks (the dev server hides this).
